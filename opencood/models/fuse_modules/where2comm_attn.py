            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        ...
                        # print(x.shape)
                        # print(communication_masks.shape)
                        ## save feature map
                        # tensor_data = communication_masks.cpu().numpy()
                        # # tensor_data = (tensor_data - tensor_data.min()) / (tensor_data.max() - tensor_data.min())
                        # fig, axs = plt.subplots(1, 4, figsize=(20, 2))
                        # for i, ax in enumerate(axs):
                        #     ax.imshow(tensor_data[i].squeeze(), cmap='gray', aspect='auto')
                        #     ax.axis('off')  # 关闭坐标轴
                        # plt.savefig("directcp_feature.png", dpi=300)
                    else:
